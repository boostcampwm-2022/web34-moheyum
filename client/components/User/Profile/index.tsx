...
import { ProfileAvatar, ProfileBio, ProfileContainer, ProfileCounters, ProfileDetail, ProfileEditButton, ProfileNames, ProfileNickname, ProfileUserid } from './index.style'
...
function UserProfile({ userData }: {userData: Props}) {
  ...
  useEffect(() => {
    //fetch Following
    httpGet(`/follow/check/${userData.userid}`).then(
      res => {
      ...
      <ProfileDetail>
        <ProfileNames>
          <ProfileNickname>
            ...
            {authedUserInfo.logined && authedUserInfo.userid !== userData.userid
            && !imfLoading && imFollowing === false &&
              <ProfileEditButton onClick={submitFollow}>
                팔로우
              </ProfileEditButton>
            }
            {authedUserInfo.logined && authedUserInfo.userid !== userData.userid
            && !imfLoading && imFollowing === true &&
              <ProfileEditButton onClick={cancleFollow}>
                팔로우 취소
              </ProfileEditButton>
            }
          </ProfileNickname>
